...
| 部署方式                         | 默认行为                                                                            |
| -------------------------------- | ----------------------------------------------------------------------------------- |
| Vercel 等非 Docker Web 部署      | 未配置自有凭证时，通过项目公共中继 `https://tv.smiling.eu.org` 加载弹弹play官方弹幕 |
...
`https://tv.smiling.eu.org` 提供弹弹play官方弹幕的公共中继能力。中继服务在服务端完成开放平台认证，下游部署和浏览器不会获得项目使用的 `AppSecret`。
