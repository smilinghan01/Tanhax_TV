...
export const changelog: ChangelogEntry[] = [
  {
    ...
    date: '2026-06-08',
    added: [
      '🏠 免登录家庭模式: 支持 NEXT_PUBLIC_AUTH_MODE=public，前台页面和常用 API 可免登录访问，后台默认仍受保护',
      '📺 番剧订阅管理: 新增订阅上下文与管理弹窗，支持订阅条目维护、状态同步与前台入口联动',
      '💬 弹弹play 开放平台集成: 接入 API 凭证管理、弹幕搜索、手动匹配与托管中继配置',
      '🎮 DecoDock 播放控制条: 新增玻璃化控制条、下一集倒计时、长按加速、环境光模式和快捷键覆盖',
      '🎚️ 视频画质筛选与分辨率处理: 新增视频质量识别、画质筛选和分辨率处理逻辑',
      '🛡️ M3U8 广告过滤能力: 新增广告片段过滤、签名代理、后台开关、按源覆盖和环境变量阈值配置',
      '📄 搜索结果加载模式: 支持触底加载与分页显示配置',
      '🔁 豆瓣代理 auto 模式: 服务端统一豆瓣请求层支持多代理自动测速、负缓存和失败降级',
    ],
    changed: [
      '⚡ 视频源测速与排序增强: 重构 EpisodeSelector 源测试反馈、延迟排序、HLS URL 检测、播放源优先级比较与验证机制',
      '🔎 搜索与播放匹配优化: 修复未知年份场景的播放页匹配，并优化搜索结果渲染、数量统计和筛选体验',
      '🧱 虚拟化视频网格优化: 引入虚拟化视频网格组件并调整列表渲染策略，降低大列表渲染开销',
      '⬇️ 下载与代理链路增强: 优化下载源规范化、请求头处理、CORS 支持、FFmpeg 下载路径和 Docker 环境下的代理请求处理',
      '🖼️ 豆瓣图片自动降级: 封面图片失败时按候选链路逐级重试，所有候选失败后才显示 poster-fallback.svg',
      '🧩 注册与管理面板优化: 后台支持控制公开注册、默认用户组和注册配置迁移',
      '📱 界面与移动端体验优化: 调整页面样式、文本对比度、过滤按钮和小屏控件显示',
      '📦 TVBox 与豆瓣集成增强: 优化 TVBox 配置、豆瓣路由、搜索联动、导航表现与广告过滤兼容性',
      '🔐 代理安全与缓存基础设施: 补充代理安全校验、统一缓存能力、分类树工具与下游请求辅助逻辑',
    ],
    fixed: [
      '🐛 修复播放页未知年份匹配、搜索结果渲染数量和筛选统计异常',
      '🐛 修复 m3u8 代理在广告过滤、上游请求头、搜索接口 URL 重写和按请求关闭过滤时的兼容问题',
      '🐛 修复豆瓣图片代理在 direct、自定义代理和 DoubanImageProxy* 配置下的 fallback 行为',
      '🐛 修复视频源测速、HLS URL 识别、源优先级比较和 EpisodeSelector UI 反馈中的稳定性问题',
      '🐛 修复 TVBox 导航与广告过滤联动问题，提升 TVBox 配置与搜索接口可用性',
